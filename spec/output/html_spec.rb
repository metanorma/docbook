...
RSpec.describe Docbook::Output::Html do
  ...
  describe "#to_html" do
    ...
    it "includes dark mode toggle" do
      skip "Frontend not built" unless frontend_built?
    ...
    it "renders emphasis without role as italic" do
      ...
      italic_child = para["children"].find { |c| %w[italic emphasis].include?(c["type"]) }
  ...
  def extract_docbook_data(html)
    ...
    data["numbering"] = if numbering_raw.is_a?(Array)
                          # NumberingEntry collection serializes as direct array
                          numbering_raw.to_h { |e| [e["id"], e["value"]] }
                        elsif numbering_raw.is_a?(Hash) && numbering_raw["entries"].is_a?(Array)
                          numbering_raw["entries"].to_h { |e| [e["id"], e["value"]] }
                        else
                          numbering_raw || {}
                        end

    # Content: convert entries array to map
    data["content"] = if content_raw.is_a?(Hash) && content_raw["entries"].is_a?(Array)
                        content_raw["entries"].to_h { |e| [e["key"], e["value"]] }
                      else
                        {}
                      end
  ...
  def frontend_built?
    File.exist?(File.join(__dir__, "..", "..", "frontend", "dist", "app.iife.js"))
  end
end
