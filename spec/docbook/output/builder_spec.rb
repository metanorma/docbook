...
RSpec.describe Docbook::Output::Builder do
  ...
  describe "#build" do
    it_behaves_like "a valid format output", :inline, ->(path) {
      html = File.read(path)
      expect(html).to include("window.DOCBOOK_DATA")
      expect(html).to include("<style>")
    }

    it_behaves_like "a valid format output", :dom, ->(path) {
      html = File.read(path)
      expect(html).to include('id="docbook-content"')
      expect(html).to include("window.DOCBOOK_FORMAT = 'dom'")
    }

    it_behaves_like "a valid format output", :dist, ->(dir) {
    ...
    it_behaves_like "a valid format output", :paged, ->(dir) {
      expect(File.exist?(File.join(dir, "index.html"))).to be true
