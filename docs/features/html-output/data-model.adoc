...
## Top-Level Model: DocbookMirror

The output uses a ProseMirror-compatible JSON document produced by `Mirror::Transformer`. The `DocbookMirror` class wraps this output:
...
| `content` | `DocbookMirror doc` | All section content as ProseMirror node trees
| `index` | `Index` | Generated index data (may be empty)
|===

[source,ruby]
----
# DocbookMirror output structure
{
  doc: {
    type: "doc",
    content: [node, node, ...]   # ProseMirror nodes
  }
}
----
...
| `value` | `SectionContent` | The section's content nodes
|===
...
| `blocks` | `DocbookMirror node[]` | Tree of DocbookMirror nodes
|===

## DocbookMirror Node Model

DocbookMirror uses ProseMirror-compatible nodes as the core building blocks for all rendered content. Each node is a recursive structure that can represent paragraphs, images, code listings, lists, admonitions, and more:

[cols="1,1,3"]
|===
| Field | Type | Description

| `type` | `String` | Node type (40+ types, see below)
| `text` | `String` | Primary text content (paragraph text, code content, title, etc.)
| `attrs` | `Object` | Node attributes (src, alt, title, language, className, etc.)
| `content` | `Node[]` | Nested child nodes (recursive)
| `marks` | `Mark[]` | Inline marks (bold, italic, code, link, etc.)
|===
