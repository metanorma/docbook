...
The Metanorma DocBook gem provides comprehensive support for DocBook XML elements through 160+ typed element classes. All element classes are implemented as `Lutaml::Model::Serializable` subclasses, providing type-safe parsing, serialization, and traversal.
...
| `Glossary` | `glossary` | Glossary section (supports `--sort-glossary`)
| `GlossEntry` | `gloss_entry` | Glossary entry with term and definition
| `GlossTerm` | (inline in `gloss_entry`) | Term being defined
| `GlossDef` | (internal) | Definition content
| `GlossSee` | `gloss_see` | "See" cross-reference to another term
| `GlossSeeAlso` | `gloss_see_also` | "See also" cross-reference
|===
...
| `Co` | Callout origin marker (inline in code blocks)
| `Callout` | Callout description referencing a `Co` marker
| `CalloutList` | List of callout descriptions
|===

Callout markers (`Co`) are extracted from code blocks during transformation.
Each `<co>` element generates a numbered marker inserted into the code text,
and its metadata (`xml_id`, `label`, `number`) is stored in the `code_block`
node's `callouts` attribute. `CalloutList` renders as a numbered annotation
list with each `Callout` item linked to its marker via `arearefs`.
...
| `QandASet` | `qandaset` | FAQ-style question-and-answer set
| `QandAEntry` | `qandaentry` | Single question-answer pair
| `Question` | `question` | The question content
| `Answer` | `answer` | The answer content
|===
...
| `Annotation` | `annotation` | Annotation with hover/click popup
| `SideBar` | `sidebar` | Sidebar content with optional title
| `Remark` | (parsed) | Editorial remark
|===

## Elements Producing HTML Output

The following elements are directly rendered as DocbookMirror node types during the output build:

- **Blocks**: `Para`, `FormalPara`, `ProgramListing`, `Screen`, `LiteralLayout`, `BlockQuote`, `SimpleSect`, `Annotation`, `SideBar`
...
- **Callouts**: `Co` (inline markers), `CalloutList`, `Callout`
- **Q&A**: `QandASet`, `QandAEntry`, `Question`, `Answer`
- **Glossary**: `Glossary`, `GlossEntry`, `GlossDef`, `GlossSee`, `GlossSeeAlso`
