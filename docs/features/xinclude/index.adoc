...
XInclude resolution is enabled by default in the `build`, `convert`, and `export` commands, and disabled by default in the `format` command:

[source,bash]
----
# XInclude enabled (default for build/convert/export)
