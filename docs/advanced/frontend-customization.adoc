...
Renders the main document content from DocbookMirror JSON. It
recursively walks the ProseMirror-compatible node tree and produces
the appropriate HTML for each node type: paragraphs, code blocks,
images, lists, admonitions, tables, and so on.

=== TextRenderer

Handles inline text nodes with marks (emphasis, code, link, xref,
citation). Used by `MirrorRenderer` for text content within block
nodes.
...
=== MirrorRenderer (reference entries)

The `MirrorRenderer` also handles `<refentry>` elements (man-page style
reference entries), rendering them with their badge, name, definition,
and description sections using the DocbookMirror node types.
...
`window.DOCBOOK_DATA` (embedded in single-page HTML). It exposes computed properties
