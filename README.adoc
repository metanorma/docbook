...
* **Output formats** -- Inline, DOM (SEO), distribution directory, and split-pages modes
...
* `--format FORMAT` -- Output format: `inline` (default), `dom`, `dist`, or `paged`
...
builder = Docbook::Output::Builder.new(
  ...
  format: :inline,             # :inline, :dom, :dist, or :paged
...
│    Paged (split pages) -- all driven by Format strategy classes       │
└────────────────────────────────────────────────────────────────────────┘
----
...
[source]
----
  docbook build    [INPUT] [-o out.html]  →  Builder → Format (inline|dom|dist|paged)
...
Builder/Formats::  The output layer. Pipeline processes XML into a guide hash,
                   then a Format strategy class (Inline, Dom, Dist, Paged) packages
                   it into the chosen output structure.
